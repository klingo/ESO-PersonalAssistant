...
function PAB_Gold.DepositGold(goldMinToKeep)
	...
	if tonumber(PAB.savedVars[activeProfile].goldDepositInterval) ~= nil then
		goldDepositInterval = PAB.savedVars[activeProfile].goldDepositInterval
	end

	-- skip rest if deposit interval not reached
	if (GetDiffBetweenTimeStamps(GetTimeStamp(), PAB.savedVars[activeProfile].goldLastDeposit) < goldDepositInterval) then
		return
	end
	
	-- calculate percentage amount to deposit
	local toDeposit = GetCurrentMoney() * (PAB.savedVars[activeProfile].goldDepositPercentage / 100)
	...
	local goldTransactionStep = tonumber(PAB.savedVars[activeProfile].goldTransactionStep)
	...
	if (toDeposit > 0) then
		...
		PAB.savedVars[activeProfile].goldLastDeposit = GetTimeStamp()
...
function PAB_Gold.WithdrawGold(goldMinToKeep)
	...
	local goldTransactionStep = tonumber(PAB.savedVars[PA.savedVars.General.activeProfile].goldTransactionStep)
