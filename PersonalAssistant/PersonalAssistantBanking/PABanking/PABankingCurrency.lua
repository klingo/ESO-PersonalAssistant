...
local function withdrawCurrency(ccyAmountToWithdraw, currencyType)
    ...
    if (ccyAmountOnBank >= ccyAmountToWithdraw) then
        ...
        if (maxCcyTransfer >= ccyAmountToWithdraw) then
            ...
            PAHF.println(GetString(SI_PA_BANKING_WITHDRAWAL_COMPLETE), ccyAmountToWithdraw, PAC.ICONS.CURRENCY[currencyType].SMALL)
        else
            -- not enough space in TARGET --> PARTIAL (limited by TARGET)
            ccyAmountToWithdraw = maxCcyTransfer
            PAHF.println(GetString(SI_PA_BANKING_WITHDRAWAL_PARTIAL_TARGET), ccyAmountToWithdraw, originalCcyAmountToWithdraw, PAC.ICONS.CURRENCY[currencyType].SMALL)
        end
    else
        -- not enough currency in SOURCE --> PARTIAL (limited by SOURCE)
        ccyAmountToWithdraw = ccyAmountOnBank
        PAHF.println(GetString(SI_PA_BANKING_WITHDRAWAL_PARTIAL_SOURCE), ccyAmountToWithdraw, originalCcyAmountToWithdraw, PAC.ICONS.CURRENCY[currencyType].SMALL)
    end
...
local function depositCurrency(ccyAmountToDeposit, currencyType)
    ...
    if (ccyAmountOnCharacter >= ccyAmountToDeposit) then
        ...
        if (maxCcyTransfer >= ccyAmountToDeposit) then
            -- enough space in TARGET --> FULL
            PAHF.println(GetString(SI_PA_BANKING_DEPOSIT_COMPLETE), ccyAmountToDeposit, PAC.ICONS.CURRENCY[currencyType].SMALL)
        else
            -- not enough space in TARGET --> PARTIAL (limited by TARGET)
            ccyAmountToDeposit = maxCcyTransfer
            PAHF.println(GetString(SI_PA_BANKING_DEPOSIT_PARTIAL_TARGET), ccyAmountToDeposit, originalCcyAmountToDeposit, PAC.ICONS.CURRENCY[currencyType].SMALL)
        end
    else
        -- not enough currency in SOURCE --> PARTIAL (limited by SOURCE)
        ccyAmountToDeposit = ccyAmountOnCharacter
        PAHF.println(GetString(SI_PA_BANKING_DEPOSIT_PARTIAL_SOURCE), ccyAmountToDeposit, originalCcyAmountToDeposit, PAC.ICONS.CURRENCY[currencyType].SMALL)
    end
